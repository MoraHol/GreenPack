...
">
  ...
  <script>
    ...
    $("input[name='material']").change(function(){
      $('#height').attr("disabled", "false")
      $('#width').attr("disabled", "false")
      $('#width').html('<option selected disabled>Seleccione</option>')
        let idMaterial = $("input[name='material']:checked").val()
        $.get('api/get_measurements_material.php',{
            idMaterial,
            idProduct
        },(data,status)=>{
            let widths = []
